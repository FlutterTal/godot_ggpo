...
int GGPO::start_spectating(Ref<GGPOSessionWrapper>& sessionRef, const String& game, int numPlayers, int localPort, String hostIp, int hostPort) {
    ...
    GGPOSession* ggpo;
    auto result = ggpo_start_spectating(&ggpo, &cb, game.utf8().get_data(), numPlayers, sizeof(uint64_t), localPort, hostIp, hostPort);
    return result;
}
...
int GGPO::synchronize_input(Ref<GGPOSessionWrapper> ggpo, Variant::INT* inputs, int length, Variant::INT& disconnectFlags) {
    callLog("synchronize_input");
    return ggpo_synchronize_input((GGPOSession*)ggpo, (uint64_t*)inputs, sizeof(uint64_t) * length, &disconnectFlags);
}

int GGPO::add_local_input(Ref<GGPOSessionWrapper> ggpo, int localPlayerHandle, Variant::INT input) {
    callLog("add_local_input");
    return ggpo_add_local_input((GGPOSession*)ggpo, localPlayerHandle, (uint64_t)&input, sizeof(uint64_t));
}
...
int GGPO::add_player(Ref<GGPOSessionWrapper> ggpo, int playerType, int playerNum, const String& playerIpAddress, Variant::INT playerPort, Variant::INT& pHandle) {
...
void GGPO::log(Ref<GGPOSessionWrapper> ggpo, const String& text) {
    callLogv("log - %s", text);
